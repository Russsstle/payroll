...
class UsersTableSeeder extends Seeder {
  ...
  public function generate($count) {
    ...
    for ($i = 0; $i < $count; $i++) {
      ...
      $profile->gender           = $faker->randomElement($array = ['Male', 'Female']);
      $profile->nationality      = 'Filipino';
      $profile->birthday         = $faker->date($format = 'Y-m-d', $max = '-20 years');
      $profile->civil_status     = 'Single';
      $profile->mobile_number    = '09' . str_pad(rand(1, 2147483647), 9, '0', STR_PAD_LEFT);
      $profile->telephone_number = $faker->tollFreePhoneNumber();
      $profile->email            = $faker->email();
      $profile->address          = 'Manila City';
      $profile->sss              = str_pad(rand(1, 2147483647), 13, '0', STR_PAD_LEFT);
      $profile->pagibig          = str_pad(rand(1, 2147483647), 13, '0', STR_PAD_LEFT);
      $profile->philhealth       = str_pad(rand(1, 2147483647), 12, '0', STR_PAD_LEFT);
